...
import system_stats
from config_validation import validate_config, unique_object
from upstream import fetch_with_deadline, MAX_UPSTREAM_BYTES
...
class SysPiper:
    ...
    def __init__(self, config_path):
        ...
        self.config = validate_config(self.load_config(config_path))
    ...
    def load_config(self, path):
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                config = json.load(f, object_pairs_hook=unique_object)
        except json.JSONDecodeError as error:
            raise ValueError(f"Invalid configuration JSON at line {error.lineno}, column {error.colno}") from None
        except (OSError, UnicodeError):
            raise ValueError("Cannot read configuration file as UTF-8") from None
    ...
    def _fetch_upstream(self, url, *, headers, timeout, verify=True,
        ...
        try:
            result = fetch_with_deadline(dict(url=url, headers=headers, timeout=timeout,
                verify=verify, expect_json=expect_json, propagate_loop=propagate_loop),
                self.config.get("upstream_deadline_seconds", 15))
        except Exception as error:
            self.logger.error("Upstream helper failed (%s)", type(error).__name__)
            abort(502, description="Upstream helper failed")
        if "error" in result:
            error = result["error"]
            self.logger.error("Upstream request failed: %s", error["description"])
            if error["code"] == 508:
                raise ProxyLoopDetected()
            # Retain the public-IP endpoint's existing transport-timeout status.
            code = 502 if not expect_json and error.get("kind") == "timeout" else error["code"]
            abort(code, description=error["description"])
        return result["value"]
    ...
    def run(self, host="0.0.0.0", port=8181):
